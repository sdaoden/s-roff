 ...
 */
#define su_FILE "dev_tty__tty"
#define rf_SOURCE
#define rf_SOURCE_DEV_TTY_TTY
...
// Horizontal position boundaries; error on excess
#define a_TTY_HPOS_MIN S16_MIN
#define a_TTY_HPOS_MAX S16_MAX

// HT (Horizontal Tabulator) - note no space conversion occurs: \t is placed!
#define a_TTY_TAB_WIDTH 8

// SGR handling (ISO 6429)
#ifndef rf_HAVE_EBCDIC
# define a_TTY_CSI "\033["
#else
# define a_TTY_CSI "\047["
#endif

#define a_TTY_SGR_BOLD a_TTY_CSI "1m"
#define a_TTY_SGR_NO_BOLD a_TTY_CSI "22m"
#define a_TTY_SGR_ITALIC a_TTY_CSI "3m"
#define a_TTY_SGR_NO_ITALIC a_TTY_CSI "23m"
#define a_TTY_SGR_UNDERLINE a_TTY_CSI "4m"
#define a_TTY_SGR_NO_UNDERLINE a_TTY_CSI "24m"
#define a_TTY_SGR_REVERSE a_TTY_CSI "7m"
#define a_TTY_SGR_NO_REVERSE a_TTY_CSI "27m"
// Many terminals cannot handle "CSI 39 m" and "CSI 49 m" to reset
// the foreground and background color, respectively; we thus use
// "CSI 0 m" exclusively
#define a_TTY_SGR_DEFAULT a_TTY_CSI "0m"

#define a_TTY_DEFAULT_COLOR_IDX -1
...
class tty_printer // XXX name
: public device::postproc
printer
{
   tty_glyph **lines;
   int nlines;
...
void tty_printer::make_underline(int w)
{
   ...
   putchar('\b');
      ...
      if (!is_underline) {
         if (italic_flag)
   putstring(a_TTY_SGR_ITALIC);
         else if (reverse_flag)
   putstring(a_TTY_SGR_REVERSE);
         else
   putstring(a_TTY_SGR_UNDERLINE);
      }
      is_underline = 1;
   }
}
...
rf_sc tty_printer::color_to_idx(color_symbol *col) // TODO col const
{
   if (col->scheme() == col->scheme_default)
      return a_TTY_DEFAULT_COLOR_IDX;
   rf_sc idx;
...
#endif
   int hpos = h / font::hor;
   if (hpos < a_TTY_HPOS_MIN || hpos > a_TTY_HPOS_MAX) {
...
{
   if (color_index == a_TTY_DEFAULT_COLOR_IDX) {
      putstring(a_TTY_SGR_DEFAULT);
...
tty_printer::page_end(int page_length)
{
   if (page_length % font::vert != 0)
      error("vertical position at end of page not multiple "
   ...
   for (int i = 0; i < last_line; i++) {
      ...
      int hpos = 0;
      tty_glyph *nextp;
      curr_fore_idx = a_TTY_DEFAULT_COLOR_IDX;
      curr_back_idx = a_TTY_DEFAULT_COLOR_IDX;
      is_underline = 0;
      is_bold = 0;
   ...
   if (horizontal_tab_flag) {
      for (;;) {
         int next_tab_pos = ((hpos + a_TTY_TAB_WIDTH) / a_TTY_TAB_WIDTH) *
               a_TTY_TAB_WIDTH;
         ...
         if (cu_flag)
            ...
            if (italic_flag)
      putstring(a_TTY_SGR_NO_ITALIC);
            else if (reverse_flag)
      putstring(a_TTY_SGR_NO_REVERSE);
            else
      putstring(a_TTY_SGR_NO_UNDERLINE);
            is_underline = 0;
         }
         putchar('\t');
         hpos = next_tab_pos;
...
font *tty_printer::font_make(const char *nm)
{
