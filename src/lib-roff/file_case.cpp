...
# undef a_X
# define a_X(L,C,E) {true, sizeof(E) -1, sizeof(C) -1, L, E, C}
...
static bool a_is_ext(a_args *ap);
...
static bool a_try_all_ext(a_args *ap);
...
#ifdef HAVE_UNPACK
  if(a.a_errno == ENOENT && (a.a_flags & mux_unpack) && a_try_all_ext(&a))
    goto jnew;
