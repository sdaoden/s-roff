...
#define GROFF_TMPDIR_ENVVAR U_ROFF_TMPDIR
...
#define TMPFILE_PREFIX_LONG L_ROFF

char *tmpfile_prefix;
size_t tmpfile_prefix_len;
int use_short_postfix = 0;

struct temp_init { /* FIXME ugh, static ctor intializer */
  temp_init();
  ~temp_init();
} _temp_init;
...
struct xtmpfile_list_init { /* FIXME ugh, static dtor finalizer */
  ~xtmpfile_list_init();
} _xtmpfile_list_init;
