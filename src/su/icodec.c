...
static u8 const a_inco_atoi[256] = {
...
static u64 const a_inco_cutlimit[35] = {
...
enum su_idec_state
su_idec_buf(void *resp, char const *cbuf, uz clen, u8 base,
      enum su_idec_mode idm, char const **endptr_or_nil){
   ...
   idm &= su__IDEC_MODE_MASK;
   rv = su_IDEC_STATE_NONE | idm;
   res = 0;
      ...
      if(base == 0){
         ...
         if(clen > 1){
            ...
            if(((c1 = cbuf[0]) >= '0' && c1 <= '9') &&
               ...
               base = a_inco_atoi[S(u8,c1)];
               if(c2 == '#')
                  c3 = cbuf[2];
               else{
                  c3 = cbuf[3];
                  base *= 10; /* xxx Inline atoi decimal base */
                  base += a_inco_atoi[S(u8,c2)];
               }
               ...
               if(base < 2 || base > 36 || a_inco_atoi[S(u8,c3)] >= base)
                  base = 10;
               else if(c2 == '#')
                  clen -= 2, cbuf += 2;
               else
...
jprefix_skip:
#if 1
               if(clen > 1 && a_inco_atoi[S(u8,cbuf[1])] < base)
                  --clen, ++cbuf;
#else
               if(*++cbuf == '\0' || --clen == 0)
                  goto jeinval;
   ...
   for(cut = a_inco_cutlimit[base - 2];;){
      if(res >= cut){
         if(res == cut){
            res *= base;
